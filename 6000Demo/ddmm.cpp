 ...
 *  OneMonitorCallback
 */
BOOL __stdcall OneMonitorCallback(HMONITOR hMonitor, HDC hdc, LPRECT prc, LPARAM lParam)
{
...
/*BOOL __stdcall FindDeviceCallback(GUID* lpGUID, LPSTR szName, LPSTR szDevice, LPVOID lParam)
{
...
BOOL __stdcall FindDeviceCallbackEx(GUID* lpGUID, LPSTR szName, LPSTR szDevice, LPVOID lParam, HMONITOR hMonitor)
{
	FindDeviceData *p = (FindDeviceData*)lParam;
...
IDirectDraw * DirectDrawCreateFromDevice(LPSTR szDevice)
{
	...
	HRESULT hr = DirectDrawEnumerateEx(FindDeviceCallbackEx, (LPVOID)&find,
 ...
 */
IDirectDraw * DirectDrawCreateFromWindow(HWND hwnd,LPGUID lpDevice)
{
    IDirectDraw *pdd;
    char szDevice[80];
    ...
    //
	//  [12/18/2009 ym] multi monitor
    //if (GetSystemMetrics(SM_CMONITORS) <= 1)
    {
		DirectDrawCreate(lpDevice, &pdd, NULL);
        return pdd;
    }
